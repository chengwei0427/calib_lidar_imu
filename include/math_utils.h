#ifndef CALIB_MATH_TUILS_H_
#define CALIB_MATH_TUILS_H_
#include <cmath>
#include <limits>
#include <Eigen/Core>
...
namespace mathutils
{
    ...
    //@beief 等同于 Matrix.eulerAngles(2, 1, 0)
    template <typename T>
    Eigen::Matrix<T, 3, 1> GetRPYFromRotationMatrix(Eigen::Matrix<T, 3, 3> R)
    {
        T roll, pitch, yaw;

        // Check that pitch is not at a singularity
        T cos_pitch = std::sqrt(R(0, 0) * R(0, 0) + R(1, 0) * R(1, 0));
        bool singular = cos_pitch < 1e-6;
